...
/// <summary>
///   hu: OwnerChangeRequest és OwnerChangeResponse szerializációs tesztek (JSON + MessagePack)
///   <br />
///   en: OwnerChangeRequest and OwnerChangeResponse serialization tests (JSON + MessagePack)
/// </summary>
public class TOwnerChangeSerializationTests
{
  ...
  #region OwnerChangeRequest - JSON..
  ...
  {
    var original = new OwnerChangeRequest("req-001", "A001B002", "1234567890123456");

    var json = JsonSerializer.Serialize(original, CJsonOptions);
    var deserialized = JsonSerializer.Deserialize<OwnerChangeRequest>(json, CJsonOptions);

    Assert.NotNull(deserialized);
    Assert.Equal(original.RequestId, deserialized.RequestId);
    Assert.Equal(original.ApNumber, deserialized.ApNumber);
    Assert.Equal(original.RegistrationNumber, deserialized.RegistrationNumber);
  }
  ...
  {
    var request = new OwnerChangeRequest("req-002", "A001B002", "1234567890123456");

    var json = JsonSerializer.Serialize(request, CJsonOptions);

    Assert.Contains("requestId", json);
    Assert.Contains("req-002", json);
    Assert.Contains("apNumber", json);
    Assert.Contains("A001B002", json);
    Assert.Contains("registrationNumber", json);
    Assert.Contains("1234567890123456", json);
  }

  #endregion

  #region OwnerChangeRequest - MessagePack..
  ...
  {
    var original = new OwnerChangeRequest("req-003", "A001B002", "1234567890123456");

    var bytes = MessagePackSerializer.Serialize(original, CMsgpackOptions);
    var deserialized = MessagePackSerializer.Deserialize<OwnerChangeRequest>(bytes, CMsgpackOptions);
  ...
  #region OwnerChangeResponse - JSON..
  ...
  {
    var original = new OwnerChangeResponse(
      RequestId: "req-004",
      ResultCode: 0,
    ...
    var json = JsonSerializer.Serialize(original, CJsonOptions);
    var deserialized = JsonSerializer.Deserialize<OwnerChangeResponse>(json, CJsonOptions);

    Assert.NotNull(deserialized);
    Assert.Equal(original.RequestId, deserialized.RequestId);
    Assert.Equal(original.ResultCode, deserialized.ResultCode);
    Assert.Equal("Teszt Kft.", deserialized.NewOperatorName);
    Assert.Equal("UNBLOCK", deserialized.BlockUnblock);
  ...
  public void Response_Json_RoundTrip_Error()
  {
    ...
    Assert.Null(deserialized.NewOperatorName);
    Assert.Null(deserialized.BlockUnblock);
  }
  ...
  {
    var response = new OwnerChangeResponse(
      RequestId: "req-006",
      ResultCode: 0
    );
    ...
    Assert.DoesNotContain("newOperatorName", json);
    Assert.DoesNotContain("blockUnblock", json);
  }

  #endregion

  #region OwnerChangeResponse - MessagePack..
  ...
  public void Response_MessagePack_RoundTrip_Success()
  {
    ...
    var bytes = MessagePackSerializer.Serialize(original, CMsgpackOptions);
    var deserialized = MessagePackSerializer.Deserialize<OwnerChangeResponse>(bytes, CMsgpackOptions);
  ...
  {
    var request = new OwnerChangeRequest("req-009", "A001B002", "1234567890123456");
  ...
  {
    var response = new OwnerChangeResponse("req-010", 0);
