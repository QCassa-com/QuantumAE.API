...
/// <summary>
///   hu: GetCertificateStatusRequest és GetCertificateStatusResponse szerializációs tesztek (JSON + MessagePack)
///   <br />
///   en: GetCertificateStatusRequest and GetCertificateStatusResponse serialization tests (JSON + MessagePack)
/// </summary>
public class TGetCertificateStatusSerializationTests
{
  ...
  #region GetCertificateStatusRequest - JSON..
  ...
  {
    var original = new GetCertificateStatusRequest("req-001");

    var json = JsonSerializer.Serialize(original, CJsonOptions);
    var deserialized = JsonSerializer.Deserialize<GetCertificateStatusRequest>(json, CJsonOptions);
  ...
  {
    var request = new GetCertificateStatusRequest("req-002");
  ...
  #region GetCertificateStatusRequest - MessagePack..
  ...
  {
    var original = new GetCertificateStatusRequest("req-003");

    var bytes = MessagePackSerializer.Serialize(original, CMsgpackOptions);
    var deserialized = MessagePackSerializer.Deserialize<GetCertificateStatusRequest>(bytes, CMsgpackOptions);
  ...
  #region GetCertificateStatusResponse - JSON..

  /// <summary>
  ///   hu: Response JSON szerializáció round-trip (sikeres válasz tanúsítvány adatokkal)
  ///   <br />
  ///   en: Response JSON serialization round-trip (successful response with certificate data)
  /// </summary>
  [Fact]
  public void Response_Json_RoundTrip_Success()
  {
    var original = new GetCertificateStatusResponse(
      RequestId: "req-004",
      ResultCode: 0,
    ...
    var json = JsonSerializer.Serialize(original, CJsonOptions);
    var deserialized = JsonSerializer.Deserialize<GetCertificateStatusResponse>(json, CJsonOptions);

    Assert.NotNull(deserialized);
    Assert.Equal(original.RequestId, deserialized.RequestId);
    Assert.Equal(original.ResultCode, deserialized.ResultCode);
    Assert.Equal(original.AuthCertExpiry, deserialized.AuthCertExpiry);
    Assert.Equal(original.SignCertExpiry, deserialized.SignCertExpiry);
    Assert.Equal(365, deserialized.DaysUntilExpiry);
  ...
  public void Response_Json_NullFieldsOmitted()
  {
    ...
    Assert.DoesNotContain("authCertExpiry", json);
    Assert.DoesNotContain("signCertExpiry", json);
    Assert.DoesNotContain("daysUntilExpiry", json);
  }

  #endregion

  #region GetCertificateStatusResponse - MessagePack..
  ...
  public void Response_MessagePack_RoundTrip_Success()
  {
    ...
    var bytes = MessagePackSerializer.Serialize(original, CMsgpackOptions);
    var deserialized = MessagePackSerializer.Deserialize<GetCertificateStatusResponse>(bytes, CMsgpackOptions);
  ...
  {
    var request = new GetCertificateStatusRequest("req-009");
  ...
  {
    var response = new GetCertificateStatusResponse("req-010", 0);
